//
//  PreChartTableViewController.swift
//  Fluctuacion
//
//  Created by jorge on 19/5/18.
...
class PreChartTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        self.performSegue(withIdentifier: "detalleBarras", sender: productos[indexPath.row])
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "detalleBarras" {
            let productoChart = sender as? Producto
            
            let nav = segue.destination as! UINavigationController
            let vc = nav.topViewController as! BarChartViewController
            vc.productoChart = productoChart
            vc.completionHandler = {(productoChart) in
                ...
                let index = self.productos.index(of: productoChart)
                if index == nil{
                    self.productos.append(productoChart)
                }else{
                    self.productos[index!] = productoChart
                }
